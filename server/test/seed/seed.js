...
const users = [{
    ...
    tokens: [{
        access: 'auth',
        token: jwt.sign({_id: userIdOne, access: 'auth'}, base64url.encode(process.env.JWT_SECRET)).toString()
    }]
}, {
    _id: userIdTwo,
    ...
    tokens: [{
        access: 'auth',
        token: jwt.sign({_id: userIdTwo, access: 'auth'}, base64url.encode(process.env.JWT_SECRET)).toString()
    }]
}]
